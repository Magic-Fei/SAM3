...
class BinaryHungarianMatcherV2(nn.Module):
    ...
    def forward(
        ...
        # Check if pred_logits is multi-class (shape [B, Q, num_classes]) or single-class (shape [B, Q, 1])
        pred_logits_shape = outputs["pred_logits"].shape
        is_multi_class = len(pred_logits_shape) == 3 and pred_logits_shape[2] > 1
        
        if is_multi_class:
            # Multi-class case: pred_logits shape is [B, Q, num_classes]
            out_score = outputs["pred_logits"]  # (B, Q, num_classes)
        else:
            # Single-class case: pred_logits shape is [B, Q, 1] or [B, Q]
        ...
        # Get target labels if available (for multi-class matching)
        tgt_labels_padded = None
        if is_multi_class and "labels_padded" in batched_targets:
            tgt_labels_padded = batched_targets["labels_padded"]
        ...
        if self.remove_samples_with_0_gt:
            ...
            if tgt_labels_padded is not None:
                tgt_labels_padded = tgt_labels_padded[batch_keep]
        ...
        if is_multi_class:
            # Multi-class case: select the probability for the target class
            if tgt_labels_padded is not None:
                # tgt_labels_padded shape: [B, max_num_targets]
                # out_prob shape: [B, Q, num_classes]
                # We need cost_class shape: [B, Q, max_num_targets]
                B, Q, num_classes = out_prob.shape
                _, max_num_targets = tgt_labels_padded.shape
                
                # For each (query, target) pair, get the probability of the target's class
                # tgt_labels_padded: [B, max_num_targets] with class indices
                # We need to gather from out_prob: [B, Q, num_classes]
                tgt_labels_expanded = tgt_labels_padded.unsqueeze(1).expand(B, Q, max_num_targets).long()  # [B, Q, max_num_targets]
                out_prob_selected = torch.gather(out_prob.unsqueeze(2).expand(B, Q, max_num_targets, num_classes), 3, tgt_labels_expanded.unsqueeze(-1)).squeeze(-1)  # [B, Q, max_num_targets]
                
                if not self.focal:
                    cost_class = -out_prob_selected
                else:
                    if self.stable:
                        rescaled_giou = (-cost_giou + 1) / 2
                        out_prob_selected = out_prob_selected * rescaled_giou
                        cost_class = -self.alpha * (1 - out_prob_selected) ** self.gamma * torch.log(
                            out_prob_selected + 1e-8
                        ) + (1 - self.alpha) * out_prob_selected**self.gamma * torch.log(1 - out_prob_selected + 1e-8)
                    else:
                        # directly computing log sigmoid (more numerically stable)
                        out_score_selected = torch.gather(out_score.unsqueeze(2).expand(B, Q, max_num_targets, num_classes), 3, tgt_labels_expanded.unsqueeze(-1)).squeeze(-1)  # [B, Q, max_num_targets]
                        log_out_prob = torch.nn.functional.logsigmoid(out_score_selected)
                        log_one_minus_out_prob = torch.nn.functional.logsigmoid(-out_score_selected)
                        cost_class = (
                            -self.alpha * (1 - out_prob_selected) ** self.gamma * log_out_prob
                            + (1 - self.alpha) * out_prob_selected**self.gamma * log_one_minus_out_prob
                        )
            else:
                # No target labels available, use max probability across classes
                # This is a fallback for when labels_padded is not provided
                out_prob_max = out_prob.max(dim=-1)[0]  # [B, Q]
                if not self.focal:
                    cost_class = -out_prob_max.unsqueeze(-1).expand_as(cost_bbox)
                else:
                    if self.stable:
                        rescaled_giou = (-cost_giou + 1) / 2
                        out_prob_max = out_prob_max.unsqueeze(-1).expand_as(cost_bbox) * rescaled_giou
                        cost_class = -self.alpha * (1 - out_prob_max) ** self.gamma * torch.log(
                            out_prob_max + 1e-8
                        ) + (1 - self.alpha) * out_prob_max**self.gamma * torch.log(1 - out_prob_max + 1e-8)
                    else:
                        out_score_max = out_score.max(dim=-1)[0]  # [B, Q]
                        log_out_prob = torch.nn.functional.logsigmoid(out_score_max)
                        log_one_minus_out_prob = torch.nn.functional.logsigmoid(-out_score_max)
                        cost_class = (
                            -self.alpha * (1 - out_prob_max) ** self.gamma * log_out_prob
                            + (1 - self.alpha) * out_prob_max**self.gamma * log_one_minus_out_prob
                        )
                        cost_class = cost_class.unsqueeze(-1).expand_as(cost_bbox)
        else:
            # Single-class case (original code)
            if not self.focal:
                cost_class = -out_prob.unsqueeze(-1).expand_as(cost_bbox)
            else:
                if self.stable:
                    ...
                    cost_class = -self.alpha * (1 - out_prob) ** self.gamma * torch.log(
                        out_prob + 1e-8
                    ) + (1 - self.alpha) * out_prob**self.gamma * torch.log(1 - out_prob + 1e-8)
                else:
                    # directly computing log sigmoid (more numerically stable)
                    log_out_prob = torch.nn.functional.logsigmoid(out_score)
                    log_one_minus_out_prob = torch.nn.functional.logsigmoid(-out_score)
...
class BinaryOneToManyMatcher(nn.Module):
    ...
    def forward(
        ...
        if is_multi_class:
            # Multi-class case: pred_logits shape is [B, Q, num_classes]
            out_prob = self.norm(outputs["pred_logits"])  # (B, Q, num_classes)
            # For multi-class, use max probability across classes
            out_prob = out_prob.max(dim=-1)[0]  # (B, Q)
        else:
            # Single-class case: pred_logits shape is [B, Q, 1] or [B, Q]
        ...
        # out_prob shape is now (B, Q) after handling multi-class case
