...
def collate_fn_api(
    ...
    for i, data in enumerate(batch):
        ...
        for j, q in enumerate(data.find_queries):
            ...
            # Get category_id from inference_metadata
            # Convert to 0-based index for loss function (if category_id is 1-based)
            category_id = q.inference_metadata.original_category_id if q.inference_metadata else -1
            # Convert 1-based category_id (1, 2, 3) to 0-based index (0, 1, 2)
            # If category_id is already 0-based or -1, keep it as is
            if category_id > 0:
                category_id_0based = category_id - 1
            else:
                category_id_0based = category_id
            for object_id in q.object_ids_output:
                ...
                # Use category_id from query metadata (same for all objects in a query)
                current_out_labels.append(category_id_0based)
            find_targets[stage_id].boxes.extend(current_out_boxes)
            find_targets[stage_id].object_ids.extend(current_out_object_ids)
            find_targets[stage_id].labels.extend(current_out_labels)
            if repeats > 0:
    ...
    for i in range(len(stages)):
        ...
        # Create labels_padded if labels exist
        if find_targets[i].labels is not None and len(find_targets[i].labels) > 0:
            find_targets[i].labels_padded = packed_to_padded_naive(
                find_targets[i].labels, find_targets[i].num_boxes, fill_value=-1
            )
        else:
            find_targets[i].labels_padded = None
