...
class IABCEMdetr(LossWithWeights):
    ...
    def get_loss(self, outputs, targets, indices, num_boxes):
        ...
        if is_multi_class:
            ...
            # For multi-class, we should use the target class label to select the corresponding logit
            # First, try to get target labels from targets
            src_logits_full = outputs["pred_logits"]  # (B, Q, num_classes)
            
            # Check if targets have labels_padded (for multi-class)
            if "labels_padded" in targets and targets["labels_padded"] is not None:
                # Get matched target labels using indices
                tgt_labels_padded = targets["labels_padded"]  # (B, max_num_targets)
                # indices[2] contains the target indices for matched pairs
                if indices[2] is not None:
                    # Get the batch indices and target indices
                    batch_idx = indices[0]  # (num_matched,)
                    tgt_idx = indices[2]  # (num_matched,)
                    # Extract matched target labels
                    matched_tgt_labels = tgt_labels_padded[batch_idx, tgt_idx]  # (num_matched,)
                    # Select the logits for matched queries and their corresponding target classes
                    query_idx = indices[1]  # (num_matched,)
                    matched_src_logits = src_logits_full[batch_idx, query_idx, matched_tgt_labels]  # (num_matched,)
                    # For all queries, use max probability as default, then replace matched ones
                    src_logits = src_logits_full.max(dim=-1)[0]  # (B, Q)
                    src_logits[batch_idx, query_idx] = matched_src_logits
                else:
                    # No matched targets, use max probability
                    src_logits = src_logits_full.max(dim=-1)[0]  # (B, Q)
            else:
                # No target labels available, use max probability across classes
                # This is a fallback but not ideal for training
                src_logits = src_logits_full.max(dim=-1)[0]  # (B, Q)
        else:
